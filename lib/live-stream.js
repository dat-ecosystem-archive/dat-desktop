...
// (obj, fn) -> null
function streamData (args, cb) {
  assert.ok(args.createArchive, 'lib/live-stream: createArchive is not defined')
  assert.ok(args.archives, 'lib/live-stream: archives is not defined')
  ...
  liveStream(args.db, {
    ...
    if (data.type === 'del') {
      ...
      // TODO close swarm
      const dat = args.archives.get(link)
      args.archives.delete(link)
      cb()
      ...
      fs.mkdir(path, () => {
        ...
        args.archives.set(link, archive)
      })
    }
    cb()
  })
