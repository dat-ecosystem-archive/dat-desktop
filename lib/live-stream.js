...
// (obj, map, obj, fn, fn) -> null
function streamData (db, archives, drive, createArchive, cb) {
  assert.ok(db, 'lib/live-stream: db is not defined')
  assert.ok(archives, 'lib/live-stream: archives is not defined')
  assert.ok(createArchive, 'lib/live-stream: createArchive is not defined')
  assert.ok(drive, 'lib/live-stream: drive is not defined')
  ...
  liveStream(db, {
    ...
    if (data.type === 'del') {
      ...
      fs.mkdir(path, () => {
        const archive = createArchive(drive, Object.assign({ key }, data.value))
        archive.open(cb)
