...
module.exports = function (dat) {
  return {
    ...
    write: function (defaults, cb) {
      ...
      // trigger an update manually until hyperdrive supports watching
      if (dat.stats) dat.stats.emit('update')

      if (dat.key) dat.metadata.url = 'dat://' + stringKey(dat.key)
      fs.writeFile(datJsonFile(dat), JSON.stringify(dat.metadata), cb)
    }
  }
}
