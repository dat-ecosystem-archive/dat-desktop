...
if (argv.throttle || argv.throttleUpload || argv.throttleDownload) {
  const throttleDown = argv.throttleDownload || argv.throttle
  const throttleUp = argv.throttleUpload || argv.throttle

  joinOpts.connect = function (local, remote) {
    if (throttleUp) local.pipe(throttle(bytes(throttleUp))).pipe(remote)
    else local.pipe(remote)

    if (throttleDown) remote.pipe(throttle(bytes(throttleDown))).pipe(local)
    else remote.pipe(local)
  }
}
