...
const joinOpts = {}
if (argv.throttle || argv.throttleUpload || argv.throttleDownload) {
  joinOpts.wrap = function (stream) {
    let input = stream
    let output = stream

    if (argv.throttleUpload || argv.throttle) {
      output = throttle(bytes(argv.throttleUpload || argv.throttle))
      stream.pipe(output)
    }

    if (argv.throttleDownload || argv.throttle) {
      input = throttle(bytes(argv.throttleDownload || argv.throttle))
      input.pipe(stream)
    }

    const throttled = duplex(input, output)

    stream.on('error', throttled.emit.bind(throttled, 'error'))
    stream.on('handshake', throttled.emit.bind(throttled, 'handshake'))

    throttled.destroy = stream.destroy.bind(stream)
