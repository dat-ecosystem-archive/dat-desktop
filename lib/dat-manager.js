...
module.exports = class Manager extends EventEmitter {
  constructor () {
    super()
    this.dats = []
    this.loadAll((err, _dats) => {
      if (err) return this.emit('error', err)
      for (let dat of _dats) this.dats.push(this.watch(dat))
      this.emit('update')
    })
  }
  watch (dat) {
  ...
  create (dir, cb) {
    ...
    dat.share(err => {
      ...
      this.persist(dat, err => {
        if (err) return cb(err)
        this.dats.push(this.watch(this.activate(dat)))
        this.emit('update')
        cb()
      })
    })
  ...
  loadAll (cb) {
    ...
    .on('data', data => {
      dats.push(this.activate(this.load(data.value)))
    })
    .on('error', err => cb(err))
    .on('end', () => cb(null, dats))
  }
  persist ({ key, owner, dir }, cb) {
    db.put(['archive', key], {
