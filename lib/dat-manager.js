...
const joinOpts = {}
if (argv.throttle || argv.up || argv.down) {
  const up = argv.up || argv.throttle
  const down = argv.down || argv.throttle

  joinOpts.connect = function (local, remote) {
    const streams = [local, remote, local]
    if (up) streams.splice(1, 0, throttle(bytes(up)))
    if (down) streams.splice(-1, 0, throttle(bytes(down)))
    pump(streams)
  }
}
